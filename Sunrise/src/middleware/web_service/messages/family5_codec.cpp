...
/** The server clock is a plain 64-bit field with no descriptor bias. */
constexpr std::uint8_t kServerClockWidth = 64;
...
bool write(encoding::bits::Writer& writer,
    ...
    for (std::size_t index = 0; encoded && index < kNestedFieldCount; ++index) {
        ...
        } else if (field == NestedField::serverClock) {
            // Absent decodes as clock 0, which parks the Client's family-5 time at the epoch.
            encoded = writer.write(1U, kPresenceWidth)
                      && writer.write(serverClockSeconds, kServerClockWidth);
